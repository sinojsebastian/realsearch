...
class ReportDynamicGeneralLedger(models.AbstractModel):
    _name = 'report.account.dynamic.report_general_ledger'
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account):
        ...
        sql = ('''SELECT l.id AS lid, l.account_id AS account_id, to_char(l.date, 'YYY-MM-DD') AS ldate, j.code AS lcode, l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
