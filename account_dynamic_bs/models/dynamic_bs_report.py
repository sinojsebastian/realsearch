...
class ReportDynamicFinancialBs(models.AbstractModel):
    ...
    def get_account_lines(self, data):
        lines = []
#         account_report = self.env.ref('accounting_pdf_reports.account_financial_report_balancesheet0')
        if data['accounts']:
            account_report = self.env['account.financial.report'].search([('id', '=', data['accounts'][0])])
        else:
            raise Warning(_(""" Please select the Balance sheet report """))
        ...
        for report in child_reports:
            ...
            vals = {
                ...
                'list_len': [a for a in range(0,bool(report.style_overwrite) and int(report.style_overwrite) or int(report.level))],
    ...
    def action_pdf(self, docids, data=None):
        data, report_lines = self.get_report_values(docids, data)
        data['form']['account_report_id'] = self.env.ref('accounting_pdf_reports.account_financial_report_balancesheet0').id
