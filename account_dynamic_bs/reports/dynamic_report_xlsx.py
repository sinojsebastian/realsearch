...
class BalanceSheetXlsx(models.AbstractModel):
    _name = 'report.account_dynamic_bs.balance_sheet_xlsx'
        ...
        if filter:
            ...
            if filter.get('date_to'):
                ...
                self.sheet.write_string(self.row_pos, 1, 'None',
                                        self.content_header)
            self.sheet.write_string(self.row_pos, 2, filter.get('target_move'),
                                    self.content_header)
            ...
            if not Filters['debit_credit'] and Filters['enable_filter']:
                ...
                for account in Accounts:
                    if int(account['level']) > 0:
                        ...
                        self.sheet.write_string(self.row_pos, 0, name + str(account.get('name')) or '',
                                                line_style)
                        self.sheet.write_string(self.row_pos, 1, account['balance'],
                                                line_style)
                        self.sheet.write_string(self.row_pos, 2, account['balance_cmp'],
                                                line_style)
