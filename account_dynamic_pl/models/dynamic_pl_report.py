...
class ReportDynamicFinancial(models.AbstractModel):
    ...
    def get_account_lines(self, data):
        lines = []
#         account_report = self.env.ref('accounting_pdf_reports.account_financial_report_profitandloss0')
        if data['accounts']:
            account_report = self.env['account.financial.report'].search([('id', '=', data['accounts'][0])])
        else:
            raise Warning(_(""" Please select the Profit and Loss account Report """))
        ...
        for report in child_reports:
            ...
            vals = {
                ...
                'list_len': [a for a in range(0,bool(int(report.style_overwrite)) and int(report.style_overwrite) or report.level)],
                'level': bool(int(report.style_overwrite)) and int(report.style_overwrite) or report.level,
    ...
    def action_pdf(self, docids, data=None):
        data, report_lines = self.get_report_values(docids, data)
        data['form']['account_report_id'] = self.env.ref('accounting_pdf_reports.account_financial_report_profitandloss0').id
        # this dict is new (for original,uncomment line 320 and comment line 321)
        ...
        dict = {
            ...
            'label_filter': data['form']['label_filter'], 
            'target_move': data['form']['target_moves'],
            'enable_filter': data['form']['enable_filter'], 
            'date_from': data['form']['date_from'],
            'date_to': data['form']['date_to'],
            'account_report_id': data['form']['account_report_id']
            }
        wizard = self.env['accounting.report'].create(dict)
